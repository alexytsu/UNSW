//code taken from http://www.sanfoundry.com/c-program-implement-heap-algorithm-permutation-n-numbers/  
    ...
    double heappermute(Set s, Set *final, List l, int v[], int n, int scaledFootRank) {
        int i;
        ...
        if (n == 1) {
            ...
            for(i = 0; i < n; i++) {
                scaledFootRank = heappermute(s, final, l, v, n-1, scaledFootRank);
                if(n % 2 == 1) {
                    permSwap(&v[0], &v[n-1]);
    	   		 }else{
                    permSwap(&v[i], &v[n-1]);
                ...
                indivScaledFoot = findScaled(l, s, n);
