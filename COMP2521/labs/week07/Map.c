...
int connections(Map g, LocationID start, LocationID end, TransportID type[])
{
    ...
    for(;curr!=NULL;curr = curr->next){
        ...
        if(curr->type == BOAT && curr->v != end){
            // find the list of connections to that sea
            VList sea = g->connections[curr->v]; 

            // search all the connections in that sea
            for(;sea!=NULL;sea = sea->next){
                if(sea->v == end){
                    type[nConns++] = sea->type;
                    break;
                }
            }   
        }
        ...
        if(curr->v == end){
            type[nConns++] = curr->type;              
        }
    }

    return nConns;  // to keep the compiler happy
}
